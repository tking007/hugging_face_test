...
def prompt_for_target_choice(max_len):
    while True:
        try:
            selected = int(input("\nEnter number of target: "))
            if 1 <= selected <= max_len:
                return selected - 1
        except Exception as e:
            ignore = e

        print("Invalid choice: Please pick a number between 1 and " + str(max_len))
...
def main():
    ...
    max_len = len(networks)
    pick = prompt_for_target_choice(max_len)
